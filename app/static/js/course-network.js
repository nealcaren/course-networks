// Course network D3.js visualization
...
async function fetchNetworkData() {
    ...
    try {
        const response = await fetch('/api/network/courses');
...
function updateGraph(data) {
    ...
    // Node size scale based on enrollment
    const nodeSizeScale = d3.scaleLinear()
        .domain([0, d3.max(nodes, d => d.enrollment) || 1])
        .range([12, 35]);
    ...
    const colorScale = d3.scaleLinear()
        .domain([0, d3.max(nodes, d => d.enrollment) || 1])
        .range(['#bbf7d0', '#4ade80']);  // Light green to medium green
    ...
    nodeMerge.select('circle')
        .attr('r', d => nodeSizeScale(d.enrollment))
        .attr('fill', d => colorScale(d.enrollment));
...
function searchNode() {
    ...
    // Match partial course names (e.g., "PSYC" or "PSYC 210")
    const foundNode = simNodes.find(n => n.id.toUpperCase().includes(searchTerm));
...
async function showNodeInfo(node) {
    ...
    html += `<div class="info-row"><span>Enrollment:</span> <strong>${node.enrollment}</strong></div>`;
    ...
    try {
        const response = await fetch(`/api/centrality/course/${encodeURIComponent(node.id)}`);
        const data = await response.json();
